...
export function canAccessTrekPhoto(requestingUserId: number, trekPhotoId: number): boolean {
  const photo = db.prepare('SELECT * FROM trek_photos WHERE id = ?').get(trekPhotoId) as
...
export async function pipeAsset(
  ...
  try {
    const resp = await safeFetch(url, { headers, signal: signal });

    response.status(resp.status);
    if (resp.headers.get('content-type')) response.set('Content-Type', resp.headers.get('content-type'));
    if (!resp.ok) {
    ...
    if (resp.headers.get('content-length')) response.set('Content-Length', resp.headers.get('content-length'));
