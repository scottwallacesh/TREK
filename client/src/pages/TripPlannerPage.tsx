...
import { accommodationsApi, addonsApi, assignmentsApi, authApi, tripsApi } from '../api/client';
...
import type { Accommodation, Day, PackingItem, Place, Reservation, TodoItem, TripMember } from '../types';
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginLeft: 'auto', flexWrap: 'wrap' }}>
                  {packingAbgehakt > 0 && (
                    <button
                      ...
                      className={`hidden items-center gap-1.5 px-[14px] py-[9px] hover:opacity-[0.88] sm:inline-flex`}
  ...
  const handleSaveReservation = async (data) => {
    try {
      if (editingReservation) {
        const r = await tripActions.updateReservation(tripId, editingReservation.id, {
          ...data,
  ...
  return (
    ...
    >
                  ...
                  onRouteCalculated={(r) => {
                    if (r) {
                      ...
                      setRouteInfo({
                        ...
                        walkingText: r.walkingText,
                        drivingText: r.drivingText,
                      });
                    } else {
                      setRoute(null);
                      setRouteInfo(null);
              ...
              <PlaceInspector
                ...
                onEdit={() => {
                  if (selectedAssignmentId) {
                    const assignmentObj = Object.values(assignments)
                      .flat()
                      .find((a) => a.id === selectedAssignmentId);
                    const placeWithAssignmentTimes = assignmentObj?.place
                      ? {
                          ...selectedPlace,
                          place_time: assignmentObj.place.place_time,
                          end_time: assignmentObj.place.end_time,
                        }
                      : selectedPlace;
                      ...
                      {mobileSidebarOpen === 'left' ? (
                        <DayPlanSidebar
                          ...
                          onRouteCalculated={(r) => {
                            if (r) {
                              setRoute(r.coordinates);
                              setRouteInfo({ distance: r.distanceText, duration: r.durationText });
                            }
                          }}
                          reservations={reservations}
                          visibleConnectionIds={visibleConnections}
      ...
      <PlaceFormModal
        ...
        dayAssignments={editingAssignmentId ? Object.values(assignments).flat() : []}
        tripId={tripId}
        categories={categories}
