...
function SpotlightStats({
  ...
  return (
    <div className="grid grid-cols-3 gap-2.5 rounded-2xl border border-white/10 bg-black/25 p-3.5 backdrop-blur-sm">
      <div className="text-center">
        <p className="text-[22px] font-extrabold tabular-nums leading-none tracking-[-0.02em]">{days}</p>
        <p className="mt-1 text-[9px] font-semibold uppercase tracking-[0.1em] opacity-70">
          {t('dashboard.mobile.days')}
        </p>
      </div>
      <div className="text-center">
        <p className="text-[22px] font-extrabold tabular-nums leading-none tracking-[-0.02em]">{places}</p>
        <p className="mt-1 text-[9px] font-semibold uppercase tracking-[0.1em] opacity-70">
          {t('dashboard.mobile.places')}
        </p>
      </div>
      <div className="text-center">
        <p className="text-[22px] font-extrabold tabular-nums leading-none tracking-[-0.02em]">{buddies}</p>
        <p className="mt-1 text-[9px] font-semibold uppercase tracking-[0.1em] opacity-70">
...
function SpotlightCard({
  ...
  const currentDay = Math.min(
    totalDays,
    Math.ceil((new Date(today).getTime() - new Date(startDate).getTime()) / 86400000) + 1
  );
  ...
  const badgeText = isLive
    ...
    : status === 'today'
      ? t('dashboard.mobile.startsToday')
      : status === 'tomorrow'
        ? t('dashboard.mobile.tomorrow')
        : status === 'future'
          ? t('dashboard.status.daysLeft', { count: daysUntil(trip.start_date) })
          : status === 'past'
  ...
  return (
    <div
      ...
      className="group relative mb-8 cursor-pointer overflow-hidden rounded-3xl transition-[transform,box-shadow] duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-1 hover:shadow-[0_16px_60px_rgba(0,0,0,0.22)] active:scale-[0.995]"
    ...
    >
        ...
        {trip.cover_image && (
          <>
            <img
              ...
              className="h-full w-full object-cover transition-transform duration-[1200ms] ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:scale-[1.06]"
              alt=""
            />
            <div
      ...
      <div
        className="absolute inset-0"
        style={{ background: 'linear-gradient(180deg, transparent 0%, transparent 40%, rgba(0,0,0,0.5) 100%)' }}
      />

      {/* Content */}
      <div className="relative z-[2] flex flex-col p-6 text-white" style={{ minHeight: 340 }}>
        {/* Top: badge + actions */}
        <div className="mb-5 flex items-center justify-between">
          {badgeText ? (
            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-black/40 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.1em] backdrop-blur-sm">
              {isLive ? (
          ...
          >
            {onEdit && (
              <button
                title={t('common.edit')}
                onClick={() => onEdit(trip)}
                className="bg-white/12 flex h-[34px] w-[34px] items-center justify-center rounded-[10px] border border-white/15 text-white backdrop-blur-sm transition-colors hover:bg-white/20"
              >
                <Edit2 size={14} />
              </button>
            )}
            ...
            {onDelete && (
              <button
                ...
                className="bg-white/12 flex h-[34px] w-[34px] items-center justify-center rounded-[10px] border border-white/15 text-red-300 backdrop-blur-sm transition-colors hover:bg-red-500/20"
              >
                <Trash2 size={14} />
              </button>
            )}
        ...
        <div className="mb-4 flex flex-1 flex-col justify-end">
          {!trip.is_owner && (
            <span className="mb-2 inline-flex items-center gap-1 self-start rounded-full border border-white/15 bg-white/15 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.06em] backdrop-blur-sm">
              <Users size={9} /> {t('dashboard.sharedBy', { name: trip.owner_username })}
            </span>
          )}
          <h2 className="mb-1.5 text-[32px] font-extrabold leading-[0.95] tracking-[-0.03em]">{trip.title}</h2>
          <p className="text-[12px] font-medium opacity-80">
            {formatDateShort(trip.start_date, locale)} — {formatDateShort(trip.end_date, locale)}
            {isLive && (
              <>
                {' '}
                · {t('journey.pdf.day')} {currentDay} / {totalDays}
              </>
            )}
          </p>
        </div>
        ...
        {isLive && (
          <div className="mb-4">
            ...
            <div className="h-1.5 overflow-hidden rounded-full bg-white/15">
              ...
              >
                <span className="absolute right-0 top-1/2 h-3 w-3 -translate-y-1/2 rounded-full bg-white shadow-[0_0_12px_rgba(255,255,255,0.9)]" />
              </div>
            </div>
          </div>
        )}
...
function MobileTripCard({
  ...
  t,
  locale,
}: Omit<TripCardProps, 'dark'>): React.ReactElement {
  const status = getTripStatus(trip);
  const until = daysUntil(trip.start_date);
  const duration =
  ...
  return (
    <div
      onClick={() => onClick?.(trip)}
      className="group cursor-pointer overflow-hidden rounded-2xl border border-zinc-200 transition-[transform,box-shadow,border-color] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-0.5 hover:shadow-md dark:border-zinc-700"
      style={{ background: 'var(--bg-card)', isolation: 'isolate' }}
    >
      {/* Cover */}
      <div
        className="relative h-[120px] overflow-hidden"
        style={{ background: trip.cover_image ? undefined : tripGradient(trip.id) }}
      >
        {trip.cover_image && (
          <img
            src={trip.cover_image}
            className="absolute inset-0 h-full w-full object-cover transition-transform duration-[800ms] ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:scale-[1.08]"
            alt=""
          />
        )}
        <div
        ...
        <div className="absolute right-3 top-3 z-[2] flex gap-1">
          {onEdit && (
            <button
              title={t('common.edit')}
              onClick={(e) => {
              ...
              className="flex h-[30px] w-[30px] items-center justify-center rounded-[8px] border border-white/20 bg-black/30 text-white backdrop-blur-sm"
            >
              <Edit2 size={12} />
            </button>
          )}
          ...
          {onDelete && (
            <button
              ...
              className="flex h-[30px] w-[30px] items-center justify-center rounded-[8px] border border-white/20 bg-black/30 text-red-300 backdrop-blur-sm"
            >
              <Trash2 size={12} />
            </button>
          )}
        ...
        {badgeText && (
          <div className="absolute left-2.5 top-2.5 z-[2]">
            <span className="inline-flex items-center gap-1 rounded-full border border-white/15 bg-black/40 px-2 py-[3px] text-[9px] font-bold uppercase tracking-[0.06em] text-white backdrop-blur-sm">
              {status === 'ongoing' ? (
                <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-red-500 shadow-[0_0_6px_rgba(239,68,68,0.8)]" />
              ) : status === 'past' ? (
        ...
        <div className="absolute bottom-3.5 left-3.5 right-3.5 z-[2] text-white">
          <h3 className="text-[22px] font-extrabold leading-none tracking-[-0.02em]">{trip.title}</h3>
          {trip.description && <p className="mt-1 truncate text-[11px] font-medium opacity-75">{trip.description}</p>}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between px-4 py-3">
        <div className="flex gap-[18px]">
          ...
          {duration && (
            <div className="flex flex-col gap-px">
              <span className="text-[13px] font-bold tracking-[-0.01em]" style={{ color: 'var(--text-primary)' }}>
                {duration} {duration === 1 ? t('dashboard.mobile.day') : t('dashboard.mobile.days')}
              </span>
              <span
                className="text-[9px] font-medium uppercase tracking-[0.06em]"
                style={{ color: 'var(--text-faint)' }}
          ...
          <div className="flex flex-col gap-px">
            ...
            <span className="text-[9px] font-medium uppercase tracking-[0.06em]" style={{ color: 'var(--text-faint)' }}>
              {t('dashboard.mobile.places')}
            </span>
          </div>
...
function TripCard({
  ...
  return (
    <div
      onClick={() => onClick(trip)}
      className="group cursor-pointer overflow-hidden rounded-2xl border border-zinc-200 transition-[transform,box-shadow,border-color] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-0.5 hover:border-zinc-300 hover:shadow-lg dark:border-zinc-700 dark:hover:border-zinc-600"
      style={{ background: 'var(--bg-card)', isolation: 'isolate' }}
    >
      {/* Cover */}
      <div
      ...
      >
        ...
        <div className="absolute right-3 top-3 z-[2] flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
          {onEdit && (
            <button
              title={t('common.edit')}
              onClick={(e) => {
              ...
              className="flex h-[30px] w-[30px] items-center justify-center rounded-[8px] border border-white/20 bg-black/30 text-white backdrop-blur-sm transition-colors hover:bg-black/50"
            >
              <Edit2 size={12} />
            </button>
          )}
          ...
          {onDelete && (
            <button
              ...
              className="flex h-[30px] w-[30px] items-center justify-center rounded-[8px] border border-white/20 bg-black/30 text-red-300 backdrop-blur-sm transition-colors hover:bg-red-500/30"
            >
              <Trash2 size={12} />
            </button>
          )}
        ...
        {/* Shared badge */}
        {!trip.is_owner && (
          <div className="absolute right-3.5 top-3.5 z-[1] transition-opacity group-hover:opacity-0">
            <span className="inline-flex items-center gap-1 rounded-full border border-white/15 bg-black/40 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.06em] text-white backdrop-blur-sm">
        ...
        {/* Title on cover */}
        <div className="absolute bottom-3.5 left-3.5 right-3.5 z-[2] text-white">
          <h3 className="text-[20px] font-extrabold leading-tight tracking-[-0.02em]">{trip.title}</h3>
          {trip.description && <p className="mt-1 truncate text-[11px] font-medium opacity-75">{trip.description}</p>}
        </div>
      </div>
...
function TripListItem({
  ...
  const coverBg = trip.cover_image ? `url(${trip.cover_image}) center/cover no-repeat` : tripGradient(trip.id);
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexShrink: 0 }}>
        {trip.start_date && (
          <div
            ...
            style={{ alignItems: 'center', gap: 4, fontSize: 12, color: 'var(--text-muted)' }}
          >
            <Calendar size={11} />
            {formatDateShort(trip.start_date, locale)}
...
function ArchivedRow({
  ...
  t,
  locale,
}: ArchivedRowProps): React.ReactElement {
  return (
    <div
      onClick={() => onClick(trip)}
      style={{
...
function IconBtn({
  ...
  return (
    <button
      ...
      onMouseEnter={(e) =>
        (e.currentTarget.style.background = danger ? 'rgba(239,68,68,0.5)' : 'rgba(255,255,255,0.25)')
      }
...
function SkeletonCard(): React.ReactElement {
  return (
    <div
      className="overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-700"
...
export default function DashboardPage(): React.ReactElement {
  ...
  const [showWidgetSettings, setShowWidgetSettings] = useState<
    boolean | 'mobile' | 'mobile-currency' | 'mobile-timezone'
  >(false);
  ...
  useEffect(() => {
    if (
      ...
      showWidgetSettings === 'mobile-currency' ||
      showWidgetSettings === 'mobile-timezone'
    ) {
        ...
        >
          ...
          <div className="mb-5 flex items-center justify-between md:hidden">
            <div>
              ...
              <p
                className="text-[22px] font-extrabold leading-tight tracking-[-0.025em]"
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full text-[15px] font-bold text-white"
          ...
          {!isLoading && spotlight && (
            <div className="mb-5 md:hidden">
              <SpotlightCard
                ...
                locale={locale}
                onEdit={
                  can('trip_edit', spotlight) || can('trip_cover_upload', spotlight)
                    ? (tr) => {
                        setEditingTrip(tr);
                        setShowForm(true);
                      }
          ...
          <div className="mb-6 grid grid-cols-3 gap-2 md:hidden">
            {can('trip_create') && (
              <button
                ...
                className="flex flex-col items-center gap-2 rounded-2xl border border-zinc-200 py-3.5 dark:border-zinc-700"
                style={{ background: 'var(--bg-card)' }}
              >
                <div
            ...
            >
              ...
              <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>
                {isLoading
                  ? t('common.loading')
                  : trips.length > 0
                    ? `${t(trips.length !== 1 ? 'dashboard.subtitle.activeMany' : 'dashboard.subtitle.activeOne', { count: trips.length })}${archivedTrips.length > 0 ? t('dashboard.subtitle.archivedSuffix', { count: archivedTrips.length }) : ''}`
          ...
          {showWidgetSettings && (
            <div
              className="mb-4 flex items-center gap-4 rounded-xl border p-3"
            ...
            >
              ...
              <label className="flex cursor-pointer items-center gap-2">
                <button
                  onClick={() => updateSetting('dashboard_currency', showCurrency ? 'off' : 'on')}
                ...
                >
                  <span
                    ...
                    style={{
                      background: 'var(--bg-card)',
                      transform: showCurrency ? 'translateX(16px)' : 'translateX(0)',
                    }}
                  />
                </button>
              ...
              <label className="flex cursor-pointer items-center gap-2">
                <button
                  onClick={() => updateSetting('dashboard_timezone', showTimezone ? 'off' : 'on')}
                  className="relative inline-flex h-5 w-9 items-center rounded-full transition-colors"
                ...
                >
                  <span
                    ...
                    style={{
                      background: 'var(--bg-card)',
                      transform: showTimezone ? 'translateX(16px)' : 'translateX(0)',
                    }}
                  />
                </button>
                <span className="text-xs" style={{ color: 'var(--text-primary)' }}>
          ...
          <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {!isLoading && rest.length > 0 && (
                <div className="mb-10 flex flex-col gap-3 md:hidden">
                  <div className="flex items-baseline justify-between px-1 pb-1">
                    <span
                      className="text-[11px] font-bold uppercase tracking-[0.12em]"
                      style={{ color: 'var(--text-faint)' }}
                    >
                      {rest.some((t) => getTripStatus(t) === 'future' || getTripStatus(t) === 'tomorrow')
                        ? t('dashboard.mobile.upcomingTrips')
                        : t('dashboard.mobile.yourTrips')}
                    </span>
                    <span className="text-[11px] font-medium" style={{ color: 'var(--text-muted)' }}>
                      {rest.length} {t('dashboard.mobile.trips')}
                    </span>
                  </div>
                  {rest.map((trip) => (
                    <MobileTripCard
                      ...
                      locale={locale}
                      onEdit={
                        can('trip_edit', trip) || can('trip_cover_upload', trip)
                          ? (tr) => {
                              setEditingTrip(tr);
                              setShowForm(true);
                            }
              ...
              {!isLoading &&
                (viewMode === 'grid' ? rest : trips).length > 0 &&
                (viewMode === 'grid' ? (
                  <div className="trip-grid trek-stagger hidden md:grid" style={{ gap: 16, marginBottom: 40 }}>
                    {rest.map((trip) => (
                      <TripCard
                        key={trip.id}
                        trip={trip}
                  ...
                  <div
                    className="trek-stagger hidden md:flex"
                    style={{ flexDirection: 'column', gap: 8, marginBottom: 40 }}
                  >
                    {trips.map((trip) => (
                      <TripListItem
                        key={trip.id}
            ...
            {showSidebar && (
              <div
                ...
                style={{ position: 'sticky', top: 80, flexShrink: 0, width: 280 }}
              >
                {showCurrency && (
                  <LiquidGlass dark={dark} style={{ borderRadius: 16 }}>
                    <CurrencyWidget />
                  </LiquidGlass>
                )}
                {showTimezone && (
        ...
        >
          ...
          >
            <div className="flex justify-center pb-2 pt-3">
              <div className="h-1 w-10 rounded-full" style={{ background: 'var(--border-primary)' }} />
            </div>
            <div className="flex items-center justify-between px-5 pb-3">
              <span className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>
                {showWidgetSettings === 'mobile-currency'
                  ? t('dashboard.mobile.currencyConverter')
                  : showWidgetSettings === 'mobile-timezone'
                    ? t('dashboard.mobile.timezone')
                    : t('common.settings')}
              </span>
