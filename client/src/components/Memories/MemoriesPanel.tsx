...
import { clearImageQueue, fetchImageAsBlob } from '../../api/authUrl';
...
export default function MemoriesPanel({ tripId, startDate, endDate }: MemoriesPanelProps) {
  ...
  const linkAlbum = async (albumId: string, albumName: string, passphrase?: string) => {
    ...
    try {
      ...
      const newLink = (linksRes.data.links || []).find(
        (l: any) => l.album_id === albumId && l.provider === selectedProvider
      );
  ...
  const removePhoto = async (photo: TripPhoto) => {
    try {
      ...
      toast.error(t('memories.error.removePhoto'));
    }
  };
  ...
  const toggleSharing = async (photo: TripPhoto, shared: boolean) => {
    try {
      ...
      setTripPhotos((prev) => prev.map((p) => (p.photo_id === photo.photo_id ? { ...p, shared: shared ? 1 : 0 } : p)));
    } catch {
      toast.error(t('memories.error.toggleSharing'));
    }
  };
      ...
      >
        ...
        <h3 style={{ margin: '0 0 6px', fontSize: 16, fontWeight: 700, color: 'var(--text-primary)' }}>
          {t('memories.notConnected', {
            provider_name: enabledProviders.length === 1 ? enabledProviders[0]?.name : 'Photo provider',
          })}
        </h3>
        <p style={{ margin: 0, fontSize: 13, color: 'var(--text-muted)', maxWidth: 300 }}>
          {enabledProviders.length === 1
            ? t('memories.notConnectedHint', { provider_name: enabledProviders[0]?.name })
            : t('memories.notConnectedMultipleHint', {
                provider_names: enabledProviders.map((p) => p.name).join(', '),
              })}
        </p>
      </div>
    );
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {albums.map((album) => {
                ...
                return (
                  <button
                    key={album.id}
                    onClick={() => !isLinked && linkAlbum(album.id, album.albumName, album.passphrase)}
                    disabled={isLinked}
                    style={{
                      display: 'flex',
                      alignItems: 'center',
  ...
  if (showPicker) {
    const alreadyAdded = new Set(
      tripPhotos.filter((p) => p.user_id === currentUser?.id).map((p) => makePickerKey(p.provider, p.asset_id))
    );
          ...
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border-secondary)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>
                {availableProviders.length > 1
                  ...
                  : t('memories.selectPhotos', {
                      provider_name:
                        availableProviders.find((p) => p.id === selectedProvider)?.name || 'Photo provider',
                    })}
              </h3>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                >
                  {selectedIds.size > 0
                    ? t('memories.addSelected', { count: selectedIds.size })
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {startDate && endDate && (
                <button
                  onClick={() => {
                    if (!pickerDateFilter) {
                      setPickerDateFilter(true);
                      loadPickerPhotos(true);
                    }
                  }}
                  style={{
                    padding: '6px 14px',
              ...
              <button
                onClick={() => {
                  if (pickerDateFilter || !startDate) {
                    setPickerDateFilter(false);
                    loadPickerPhotos(false);
                  }
                }}
                style={{
                  padding: '6px 14px',
              ...
              (() => {
                ...
                return sortedMonths.map((month) => (
                  <div key={month} style={{ marginBottom: 16 }}>
                    ...
                    <div
                      style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 4 }}
                    >
                      {byMonth[month].map((asset) => {
                        ...
                        return (
                          ...
                          >
                            <ProviderImg
                              baseUrl={buildProviderAssetUrlFromAsset(asset, 'thumbnail', currentUser!.id)}
          ...
          >
            {albumLinks.map((link) => (
              ...
              >
                ...
                {link.username !== currentUser?.username && (
                  <span style={{ color: 'var(--text-faint)' }}>({link.username})</span>
                )}
                <button
                  onClick={() => syncAlbum(link.id, link.provider)}
                ...
                >
                  <RefreshCw
                    size={11}
                    style={{ animation: syncing === link.id ? 'spin 1s linear infinite' : 'none' }}
                  />
                </button>
                {link.user_id === currentUser?.id && (
                  <button
                    onClick={() => unlinkAlbum(link.id)}
                    title={t('memories.unlinkAlbum')}
                    style={{
                      background: 'none',
                      border: 'none',
                      cursor: 'pointer',
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    apiClient
                      .get(buildProviderAssetUrl(photo, 'info'))
                      .then((r) => setLightboxInfo(r.data))
                      .catch(() => {})
                      .finally(() => setLightboxInfoLoading(false));
                  }}
                >
                  <ProviderImg
                  ...
                  {isOwn && (
                    ...
                    >
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleSharing(photo, !photo.shared);
                        }}
        ...
        (() => {
              ...
              {lightboxInfo.takenAt && (
                <div>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 600 }}>
                    {new Date(lightboxInfo.takenAt).toLocaleDateString(undefined, {
                      day: 'numeric',
                      month: 'long',
                      year: 'numeric',
                    })}
                  ...
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>
                    {new Date(lightboxInfo.takenAt).toLocaleTimeString(undefined, {
                      hour: '2-digit',
                      minute: '2-digit',
                    })}
                  </div>
          ...
          return (
            <div
              ...
              onKeyDown={(e) => {
                if (e.key === 'ArrowLeft' && hasPrev) navigateTo(currentIdx - 1);
                if (e.key === 'ArrowRight' && hasNext) navigateTo(currentIdx + 1);
              ...
              onTouchEnd={(e) => {
                const start = (e.currentTarget as any)._touchX;
                if (start == null) return;
                const diff = e.changedTouches[0].clientX - start;
                if (diff > 60 && hasPrev) navigateTo(currentIdx - 1);
                else if (diff < -60 && hasNext) navigateTo(currentIdx + 1);
              }}
              style={{
                position: 'absolute',
                inset: 0,
