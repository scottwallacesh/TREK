...
export default function OfflineTab(): React.ReactElement {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [metas, pending] = await Promise.all([offlineDb.syncMeta.toArray(), mutationQueue.pendingCount()]);
    ...
    <Section title="Offline Cache" icon={Database}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {loading ? (
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>No trips cached yet. Connect to internet to sync.</p>
        ) : (
            ...
            {rows.map(({ trip, meta, placeCount, fileCount }) => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontWeight: 600, fontSize: 14, color: 'var(--text-primary)' }}>{trip.name}</span>
