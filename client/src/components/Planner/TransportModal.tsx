import { Car, ExternalLink, FileText, Link2, Paperclip, Plane, Ship, Train, X } from 'lucide-react';
...
export function TransportModal({
  ...
  useEffect(() => {
    ...
    if (reservation) {
      ...
      setForm({
        ...
        price: meta.price || '',
        budget_category:
          meta.budget_category && budgetItems.some((i) => i.category === meta.budget_category)
            ? meta.budget_category
            : '',
      });
      if (type === 'flight') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.type === 'flight') {
        if (fromPick.airport)
          endpoints.push(endpointFromAirport(fromPick.airport, 'from', 0, startDate, form.departure_time || null));
        if (toPick.airport)
          endpoints.push(endpointFromAirport(toPick.airport, 'to', 1, endDate, form.arrival_time || null));
      } else {
      ...
      if (isBudgetEnabled) {
        (payload as any).create_budget_entry =
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            ...
            {form.type === 'flight' ? (
              <AirportSelect
                value={fromPick.airport || null}
                onChange={(a) => setFromPick({ airport: a || undefined })}
              />
            ) : (
              <LocationSelect
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <label style={labelStyle}>
              {form.type === 'flight'
                ? t('reservations.arrivalTime')
                : form.type === 'car'
                  ? t('reservations.returnTime')
        ...
        {form.type === 'flight' && (
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              ...
              <input
                type="text"
                value={form.meta_airline}
                onChange={(e) => set('meta_airline', e.target.value)}
            ...
            <div>
              ...
              <input
                type="text"
                value={form.meta_flight_number}
                onChange={(e) => set('meta_flight_number', e.target.value)}
                placeholder="LH 123"
                style={inputStyle}
              />
            </div>
            ...
            {attachedFiles.map((f) => (
              ...
              >
                ...
                <a
                  ...
                  style={{ color: 'var(--text-faint)', display: 'flex', flexShrink: 0, cursor: 'pointer' }}
                >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (v === '' || /^\d*[.,]?\d{0,2}$/.test(v)) set('price', v.replace(',', '.'));
                  }}
                  onPaste={(e) => {
                    e.preventDefault();
                    let txt = e.clipboardData
                      .getData('text')
                      .trim()
                      .replace(/[^\d.,-]/g, '');
                    const lc = txt.lastIndexOf(','),
                      ld = txt.lastIndexOf('.'),
                      dp = Math.max(lc, ld);
                    if (dp > -1) {
                      txt = txt.substring(0, dp).replace(/[.,]/g, '') + '.' + txt.substring(dp + 1);
                    } else {
                      txt = txt.replace(/[.,]/g, '');
                    }
                    set('price', txt);
